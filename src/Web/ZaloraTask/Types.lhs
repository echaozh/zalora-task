...
`MonadRead` instance for read-only database connection pool and photo storage
path setting access is more than enough. For clearance, I call read-only data
...
I am trying out the unicode syntax, so I always import the modules from
...
`ScottyError` class. For now, unless you go to wild pages out of control of the
`Controller` and land on a Scotty generated 404 page, you will only see the
